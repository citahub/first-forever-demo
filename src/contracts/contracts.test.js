...
test(`Add record of (${text}, ${time})`, async () => {
  ...
  const txResult = await simpleStoreContract.methods
    .add(text, time)
    .send(transaction); // sendTransaction to the contract
  const receipt = await cita.listeners.listenToTransactionReceipt(
    txResult.hash
  ); // listen to the receipt
