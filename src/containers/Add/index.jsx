  ...
  handleSubmit = e => {
    ...
    .then(res => {
      if (res.hash) {
        ...
        return cita.listeners.listenToTransactionReceipt(res.hash).then(
            receipt => {
    ...
    return (
        <div className="add__content--container">
          <div className="add__time--container">
            <span
                className="add__time--year"> {time.getFullYear()} </span>:{' '}
            <span className="add__time--month"> {timeFormatter(
                (time.getMonth() + 1) % 12)} </span>:{' '}
            <span className="add__time--day"> {timeFormatter(
                time.getDate())} </span>:{' '}
            <span className="add__time--hour"> {timeFormatter(
                time.getHours())} </span>:{' '}
            <span className="add__time--min"> {timeFormatter(
